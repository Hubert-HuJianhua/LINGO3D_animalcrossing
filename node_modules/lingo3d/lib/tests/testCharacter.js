...
//     cam.innerX = 20 * val
// }
let camPoint = cam.rayAt(10000);
cam.onLoop = () => {
    camPoint = cam.rayAt(10000);
    camPoint.y += 2000;
    model.lookAt(new Point3d(camPoint.x, 0, camPoint.z));
    cam.x = model.x;
    cam.z = model.z;
    cam.y = model.y;
};
...
const useFireBolt = (aim) => createEffect(() => {
    // if (aim)
    //     camSpring.to = 1.5
    // else
    //     camSpring.to = 1
...
const useDetectLanding = (airborn) => createEffect(() => {
    if (!airborn)
        return;
